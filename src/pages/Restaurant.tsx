...
export const Restaurant: React.FC = () => {
  ...
  const loadProducts = async () => {
    try {
      ...
      setProducts(allProducts.filter(p => p.location === 'restaurant'));
    } catch (error) {
  ...
  return (
    <div className="p-4 lg:p-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <h1 className="text-2xl lg:text-3xl font-bold">Restaurant Inventory</h1>
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <p className="text-2xl font-bold text-blue-600">R{getTotalValue(filteredProducts)}</p>
        </CardContent>
      </Card>
      ...
      {filteredProducts.length === 0 && (
        <div className="text-center py-12">
          <p className="text-muted-foreground">No restaurant products found.</p>
        </div>
      )}
    </div>
  );
